...
interface StatusLifecycleEntry {
  messageId: string;
  preview: string;
  stage: string;
  reason?: string;
...
export interface StatusInfo {
  ...
  recentMessages?: StatusLifecycleEntry[];
}
...
export function statusCard(info: StatusInfo): object {
  ...
  const recent = info.recentMessages ?? [];
  if (recent.length > 0) {
    lines.push(
      '',
      '**最近消息**:',
      ...recent.map((entry) => {
        ...
        const preview = entry.preview ? ` — ${escapeMd(entry.preview)}` : '';
        return `- \`${escapeCode(entry.messageId.slice(0, 10))}…\` ${escapeMd(entry.stage)}${reason}${preview}`;
      }),
    );
  }
  return shell('📊 当前状态', [
  ...
  return shell('💡 使用帮助', [
    divMd(
      [
        ...
        '- `/lark-auth <domain|scope ...>` `/lark-auth done|status` — 两段式完成 lark-cli 用户授权',
        '- `/status` — 当前状态',
        ...
        '- `/schedule add|list|remove|help` — 定时任务（bridge 常驻时到点跑 prompt 并发回本会话）',
